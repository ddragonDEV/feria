...
class ChatsController extends Controller
{
	...
	*/
	public function agregarAction(Request $request){
		...
		$oStands = $em->getRepository('AdminBundle:Stands')->findBy(array('idEstado' => 1));
		...
		if ($request->getMethod() === 'POST') {
			...
			if($nombre && $diasFuncionamiento && $horaApertura && $horaCierre){
				...
				$oChats->setIdPunto($em->getReference('AdminBundle:Puntos', $oStands->getIdPunto()->getId()));
			...
			$errores = array(
				...
				"id_punto"              => $id_punto ? "": "Debe seleccionar un id_punto"
			);
	...
	*/
	...
	public function editarAction(Request $request){
		...
		if ($request->getMethod() === 'POST') {
			...
			if($nombre && $diasFuncionamiento && $horaApertura && $horaCierre && $id_punto){

				$oChats->setNombre($nombre);
				$oChats->setDiasFuncionamiento(implode(',', $diasFuncionamiento));
				$oChats->setHoraApertura($oHoraApertura);
				$oChats->setHoraCierre($oHoraCierre);
				$oChats->setIdPunto($em->getReference('AdminBundle:Puntos', $id_punto));

				$oChats->setIdStand(null);
			...
			$errores = array(
				...
				"horaApertura"          => $horaApertura ? "":"La hora de apertura es obligatoria",
				"horaCierre"            => $horaCierre ? "":"La hora de cierre es obligatoria",
				"id_punto"              => $id_punto ? "":"El id_punto es obligatorio"
			);
		...
		$valores = array(
			...
			"id_punto"              => $oChats->getIdPunto()->getId(),
